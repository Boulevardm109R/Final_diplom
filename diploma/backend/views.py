...
class CustomAuthToken(ObtainAuthToken):
    ...
    при запросе проверяет тип пользователя, если магазин, то выдает/создает токен пользователю
...
class SignUpView(CreateView):
    ...
    пользователь при регистарции может указать свой тип самостоятельно, от этого зависит получит ли он токен-ключ или нет на почту
...
class RequestForChangePasswordView(APIView):
    throttle_classes = [UserRateThrottle]
    """
    класс отправляет сообщение с ссылкой на изменение пароля
